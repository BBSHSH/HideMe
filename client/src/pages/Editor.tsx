...
export default function HideMeEditor() {
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        .glass-panel:hover { border-color: rgba(190, 194, 255, 0.3); }
        .vault-gradient {
          background: linear-gradient(180deg, rgba(88, 101, 242, 0.1) 0%, rgba(18, 19, 27, 0) 100%);
        }
        input[type=range] { accent-color: #bec2ff; }
        select { color: #e3e1ed; }
        select option { background: #292932; }
        @keyframes pulse {
          0%, 100% { opacity: 1; }
          50% { opacity: 0.5; }
        }
      ...
      <div style={{ position: "fixed", bottom: 16, right: 16, zIndex: 50 }}>
        <div
          className="glass-panel"
          style={{ padding: "8px 16px", borderRadius: 8, display: "flex", alignItems: "center", gap: 8, borderColor: "rgba(190,194,255,0.2)", boxShadow: "0 8px 32px rgba(0,0,0,0.4)" }}
        >
          <span className="material-symbols-outlined animate-pulse" style={{ color: "#bec2ff" }}>security</span>
          <div style={{ display: "flex", flexDirection: "column" }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: "#bec2ff" }}>PERIMETER: SECURE</span>
            <span style={{ fontSize: 9, color: "#c6c5d7" }}>監視ログ有効化済み</span>
          </div>
        </div>
      </div>
    </div>
